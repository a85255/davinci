...
export class SourceList extends React.PureComponent<ISourceListProps, ISourceListStates> {
  ...
  private editSource = (sourceId: number) => () => {
    this.props.onLoadSourceDetail(sourceId, (editingSource) => {
      let url = editingSource.config.url
      let Str = url.split('//')[1]
      let type = url.split(':')[1]
      if (type === 'oracle') {
        editingSource.config.databaseType = 'oracle'
        if (Str.indexOf('/') > -1) {
          editingSource.config.sid = Str.split('/')[1]
          Str = Str.split('/')[0]
        }
        if (Str.indexOf(':') > -1) {
          editingSource.config.port = Str.split(':')[1]
          Str = Str.split(':')[0]
        }
        editingSource.config.host = Str
      } else if (type === 'mysql') {
        editingSource.config.databaseType = 'mysql'
        if (Str.indexOf('/') > -1) {
          editingSource.config.sid = Str.split('/')[1]
          Str = Str.split('/')[0]
        }
        ...
        editingSource.config.databaseType = 'sqlserver'
        if (Str.indexOf(';databaseName=') > -1) {
          editingSource.config.sid = Str.split(';databaseName=')[1]
          Str = Str.split(';databaseName=')[0]
        }
        if (Str.indexOf(':') > -1) {
          editingSource.config.port = Str.split(':')[1]
          Str = Str.split(':')[0]
  ...
  private saveSourceForm = (values: ISource) => {
    ...
    let config = requestValue.config
    if (config.databaseType === 'oracle') {
      config.url = `jdbc:oracle:thin:${config.username}/${config.password}@//${config.host}`
      config.url = config.port ? config.url + ':' + config.port : config.url
      config.url = config.sid ? config.url + '/' + config.sid : config.url
    } else if (config.databaseType === 'mysql') {
      config.url = `jdbc:mysql://${config.host}`
      config.url = config.port ? config.url + ':' + config.port : config.url
      config.url = config.sid ? config.url + '/' + config.sid : config.url
    } else if (config.databaseType === 'sqlserver') {
      config.url = `jdbc:sqlserver://${config.host}`
      config.url = config.port ? config.url + ':' + config.port : config.url
      config.url = config.sid ? config.url + ';databaseName=' + config.sid : config.url
    }
    if (!values.id) {
  ...
  private testSourceConnection = (config) => {
    if (config.host === '') {
      message.error('服务器地址 不能为空')
    } else {
      let url
      if (config.databaseType === 'oracle') {
        url = `jdbc:oracle:thin:${config.username}/${config.password}@//${config.host}`
        url = config.port ? url + ':' + config.port : url
        url = config.sid ? url + '/' + config.sid : url
      } else if (config.databaseType === 'mysql') {
        url = `jdbc:mysql://${config.host}`
        url = config.port ? url + ':' + config.port : url
        url = config.sid ? url + '/' + config.sid : url
      } else if (config.databaseType === 'sqlserver') {
        url = `jdbc:sqlserver://${config.host}`
        url = config.port ? url + ':' + config.port : url
        url = config.sid ? url + ';databaseName=' + config.sid : url
      }
