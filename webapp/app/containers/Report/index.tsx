...
export function mapDispatchToProps (dispatch) {
  return {
    onPageLoad: () => {
      const sidebarSource = [
        { icon: (<i className="iconfont icon-dashboard" />), route: ['vizs', 'dashboard'], permission: 'viz', title: '看板展示' },
        { icon: (<i className="iconfont icon-widget-gallery" />), route: ['widgets'], permission: 'widget', title: '图表管理' },
        { icon: (<i className="iconfont icon-custom-business" />), route: ['views', 'view'], permission: 'view', title: '视图管理' },
        { icon: (<i className="iconfont icon-datasource24" />), route: ['sources'], permission: 'source', title: '数据源管理' },
